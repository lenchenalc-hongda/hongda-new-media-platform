...
const HOOK_FRAGMENTS: Record<string, string[]> = {
  ...
  material: [
    'PE瓶能不能做热转印？这句话不能直接回答。',
    '只说"塑料瓶"，为什么我要继续问材质？',
    '不锈钢杯子做热转印，前提是什么？',
    'ABS、PP、PE都叫塑料，热转印效果能差一倍。',
  ],
  test: [
    '你问我会不会掉，我最怕直接回答"不会"。',
  ...
  color: [
    '客户说颜色按图片做，这个风险很大。',
    '手机上看到的颜色，不等于印出来的颜色。',
    '你说"就这个颜色"，我得先问有没有潘通色号。',
    '金色、银色、幻彩色，不是普通印刷能做的。',
  ],
  sample: [
    '打样和大货做到一模一样，真的那么简单吗？',
    '客户说"不打样直接做大货"，为什么有些订单不能接？',
    '免费打样，不是所有客户都能享受的。',
    '商样确认后换材料，为什么要重新打样？',
  ],
  small_batch: [
...
export function generateHook(input: {
  ...
  const catMap: Record<string, string> = {
    ...
    '客户误区': 'misunderstanding', '工艺': 'process', '防背粘': 'anti_back',
  };
  ...
  const chineseChars = hook.match(/[\u4e00-\u9fff]/g);
...
function getChunkType(input: any): string {
  const { customerPain, productOrProcess, knowledgeCards } = input;
  const pain = customerPain || '';
  const product = productOrProcess || '';
  const firstCard = knowledgeCards?.[0];
  const cardTitle = firstCard?.title || '';

  if (cardTitle.includes('防背粘') || product.includes('防背粘')) return 'anti_back';
  if (pain.includes('价格') || pain.includes('多少钱') || pain.includes('报价')) return 'price';
  if (pain.includes('材质') || pain.includes('PE') || pain.includes('PP') || pain.includes('ABS') || pain.includes('材料')) return 'material';
  if (pain.includes('测试') || pain.includes('附着力') || pain.includes('掉') || pain.includes('酒精')) return 'test';
  if (pain.includes('颜色') || pain.includes('色差') || pain.includes('图片')) return 'color';
  if (pain.includes('打样') || pain.includes('样品') || pain.includes('确认样')) return 'sample';
  if (pain.includes('误') || pain.includes('注意') || pain.includes('常见')) return 'misunderstanding';
  if (cardTitle.includes('花膜结构') || product.includes('花膜')) return 'process';
  return 'price';
}
...
const BATCH_ANGLES: { angle: string; category: string; pain: string }[] = [
  { angle: '价格角度', category: 'price', pain: '客户问多少钱' },
  { angle: '材质角度', category: 'material', pain: '材质能不能做热转印' },
  { angle: '测试角度', category: 'test', pain: '附着力测试' },
  { angle: '颜色角度', category: 'color', pain: '颜色按图片做' },
  { angle: '打样角度', category: 'sample', pain: '打样和大货不一致' },
  { angle: '小批量角度', category: 'small_batch', pain: '小批量能不能做' },
  { angle: '客户误区', category: 'misunderstanding', pain: '客户只发图片判断不了' },
  { angle: '工艺角度', category: 'process', pain: '热转印花膜结构' },
];
