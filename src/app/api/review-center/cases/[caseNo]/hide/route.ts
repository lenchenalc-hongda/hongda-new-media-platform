...
import { caseHideRequestSchema, caseNoSchema } from '@/lib/review-center/case-schemas';
import { callCaseHide } from '@/lib/review-center/case-rpc';
...
export async function POST(
  ...
  try {
    ...
    const parsed = caseHideRequestSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ error: '请求参数无效' }, { status: 400 });
    }
    const supabase = await createClient();
    if (!supabase) return toCaseRouteErrorResponse(new Error('supabase unavailable'), { route: 'case-hide' });
    const caseId = await resolveAdminCaseId(supabase, parsedCaseNo.data);
    const result = await callCaseHide(supabase, caseId, parsed.data);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    return toCaseRouteErrorResponse(err, { route: 'case-hide' });
  }
}
