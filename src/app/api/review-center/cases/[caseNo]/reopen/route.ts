...
import { caseNoSchema, caseReopenRequestSchema } from '@/lib/review-center/case-schemas';
import { callCaseReopen } from '@/lib/review-center/case-rpc';
...
export async function POST(
  ...
  try {
    ...
    const parsed = caseReopenRequestSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ error: '请求参数无效' }, { status: 400 });
    }
    const supabase = await createClient();
    if (!supabase) return toCaseRouteErrorResponse(new Error('supabase unavailable'), { route: 'case-reopen' });
    const caseId = await resolveAdminCaseId(supabase, parsedCaseNo.data);
    const result = await callCaseReopen(supabase, caseId, parsed.data);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    return toCaseRouteErrorResponse(err, { route: 'case-reopen' });
  }
}
