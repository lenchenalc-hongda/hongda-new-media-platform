...
import { callCaseAdminDetail } from '@/lib/review-center/case-rpc';
...
export async function GET(
  ...
  try {
    ...
    if (!supabase) return toCaseRouteErrorResponse(new Error('supabase unavailable'), { route: 'case-admin' });
    const result = await callCaseAdminDetail(supabase, parsed.data);
    return NextResponse.json({ ok: true, data: result });
  } catch (err) {
    return toCaseRouteErrorResponse(err, { route: 'case-admin' });
  }
}
